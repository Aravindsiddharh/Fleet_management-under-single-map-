...
#         def _maybe_namespace(frame: str) -> str:
#             # Keep global map frame exactly as configured
#             if frame == self.map_frame:
#                 return self.map_frame

#             # If it's already namespaced for this robot, leave it alone
#             if frame.startswith(f'{self.robot_name}/'):
#                 return frame

#             # If it's already a different namespace, leave it alone (assume external)
#             if '/' in frame and not frame.startswith(self.robot_name + '/'):
#                 return frame

#             # Otherwise treat it as robot-local and namespace it
#             return namespacify(frame, self.robot_name)
...
#                 # Apply controlled namespacing rules
#                 t.header.frame_id = _maybe_namespace(zt.header.frame_id)
#                 t.child_frame_id = _maybe_namespace(zt.child_frame_id)

#                 # Use robot-specific authority to avoid collision in tf_buffer
#                 try:
#                     self.tf_buffer.set_transform(t, f'{self.robot_name}_TfListener')
#                 except Exception as e:
#                     self.node.get_logger().warn(
#                         f'Failed to set transform in buffer: {type(e)}: {e}'
#                     )

#         # subscribe to the robot's TF channel in zenoh (unchanged)
#         self.tf_sub = self.zenoh_session.declare_subscriber(
#             #namespacify('tf', self.robot_name),
#             'tf',
#             _tf_callback
...
#     def get_transform(self) -> TransformStamped | None:
#         """
#         Return a TransformStamped giving robot_frame in map coordinates.
#         Uses a safe 'latest' timestamp to avoid common extrapolation issues.
#         """
#         try:
#             target_frame = self.map_frame  # NEVER namespace the map
#             # robot_frame may already be namespaced in config; namespace if not
#             if self.robot_frame.startswith(f'{self.robot_name}/'):
#                 source_frame = self.robot_frame
#             else:
#                 source_frame = namespacify(self.robot_frame, self.robot_name)

#             # Use the latest available time from the node clock to reduce extrapolation errors
#             when = self.node.get_clock().now()

#             transform = self.tf_buffer.lookup_transform(
#                 target_frame,
#                 source_frame,
#                 rclpy.time.Time()  # safer than an unbounded zero timestamp for many setups
#             )
#             return transform
#         except Exception as err:
#             self.node.get_logger().info(
#                 f'Unable to get transform from [{source_frame}] to [{target_frame}]: '
#                 f'{type(err)}: {err}'
#             )
#         return None
...
# Added imports for timing and tf timeout
import time
...
class Nav2TfHandler:
    ...
    def get_transform(self) -> TransformStamped | None:
        """
        Return a TransformStamped giving robot_frame in map coordinates.
        Uses a safe 'latest' timestamp and waits a short time for TF to avoid
        ...
        try:
            ...
            # Wait up to 0.5s for a transform to appear (avoids extrapolation)
            wait_ok = self.tf_buffer.can_transform(
                target_frame,
                source_frame,
                rclpy.time.Time(),
            ...
            if not wait_ok:
                # No fresh transform yet — caller should try again later
                return None

            # Use the latest transform (Time() = latest) which is the safest option in many setups
            transform = self.tf_buffer.lookup_transform(
                target_frame,
                source_frame,
                rclpy.time.Time()
            ...
            # source_frame may not be defined here in exceptional paths; guard its usage
            try:
                src = source_frame
            except Exception:
                src = self.robot_frame
            self.node.get_logger().info(
                f'Unable to get transform from [{src}] to [{target_frame}]: '
                f'{type(err)}: {err}'
            )
        return None
...
class Nav2RobotAdapter(RobotAdapter):
    ...
    def __init__(
        ...
        # For duplicate goal suppression
    ...
    def update(self, state: rmf_easy.RobotState):
        ...
        if exec_handle:
            ...
            if exec_handle.execution and exec_handle.goal_id and \
                ...
                exec_handle.execution.finished()
                exec_handle.execution = None
                # small stabilization: give the robot driver a moment to settle to avoid tiny pose jitter
                try:
                    time.sleep(0.15)
                except Exception:
                    pass
        ...
        for reply in replies:
            try:
                ...
                if rep.accepted:
                    ...
                    nav_handle.set_goal_id(nav_goal_id)
                    # record last goal to avoid tight re-sends of identical goals
                    try:
                        goal_key = (self.map_name, round(x, 3), round(y, 3), round(yaw, 3))
                        self._last_goal = goal_key
                        self._last_goal_time = self.node.get_clock().now().seconds_nanoseconds()[0]
                    except Exception:
                        pass
                    return
    ...
    def navigate(
        ...
        # duplicate-goal suppression: avoid sending the same exact goal in a very short window
        try:
            goal_key = (destination.map, round(destination.position[0], 3),
                        round(destination.position[1], 3), round(destination.position[2], 3))
            now_s = self.node.get_clock().now().seconds_nanoseconds()[0]
            if self._last_goal == goal_key and (now_s - self._last_goal_time) < 3.0:
                self.node.get_logger().info('Ignoring repeated identical goal in short window')
                return
        except Exception:
            pass
    ...
    def _handle_stop_navigation(self):
        ...
        # short wait to allow local controller / driver to settle after cancel
        try:
            time.sleep(0.20)
        except Exception:
            pass
